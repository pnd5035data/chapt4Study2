...
async function buildExperimentTrials(config, genderCondition) {
  ...
  // Map videos with new column names and typicalityCode
  const maleVideos = maleRows.map((row, idx) => ({
    id: `M${idx + 1}`,
    url: row.videoFile,
    gender: "M",
    typicality: row.typicalityCode
  })).filter(v => v.url);

  const femaleVideos = femaleRows.map((row, idx) => ({
    id: `F${idx + 1}`,
    url: row.videoFile,
    gender: "F",
    typicality: row.typicalityCode
  ...
  // Select videos based on gender condition
  const selectedVideos = genderCondition === "M" ? maleVideos : femaleVideos;
  const typVideos = selectedVideos.filter(v => v.typicality === "typ");
  const atypVideos = selectedVideos.filter(v => v.typicality === "atyp");

  if (!typVideos.length) throw new Error(`No typical videos found for gender ${genderCondition}`);
  if (!atypVideos.length) throw new Error(`No atypical videos found for gender ${genderCondition}`);

  const ratioConditions = conditionRows
    .filter(row => String(row.video_trial || "1") === "1")
    .map((row, index) => ({
      conditionIndex: index,
      numTyp: Number(row.num_typ),
      numAtyp: Number(row.num_atyp),
      ratioLabel: `${row.num_typ}:${row.num_atyp}`,
      sourceRow: row
    }));
  ...
  const baseVideoSets = ratioConditions.map(condition => {
    const typSet = sampleWithoutReplacementIfPossible(typVideos, condition.numTyp);
    const atypSet = sampleWithoutReplacementIfPossible(atypVideos, condition.numAtyp);
    return shuffleArray([...typSet, ...atypSet]);
  });
    ...
    for (const condition of randomizedConditions) {
      for (let repetition = 0; repetition < 5; repetition++) {
        ...
        const videosForThisTrial = shuffleArray(baseVideos).map((videoMeta, slotIndex) => ({
          ...
          typicality: videoMeta.typicality,
        ...
        trials.push({
          trialId: `B${blockSpec.blockId}_C${condition.conditionIndex + 1}_R${repetition + 1}_${condition.ratioLabel}`,
          meta: {
            ...
            numTyp: condition.numTyp,
            numAtyp: condition.numAtyp,
  ...
  return {
    ...
    buildInfo: {
      ...
      typVideoCount: typVideos.length,
      atypVideoCount: atypVideos.length,
...
class ExperimentController {
  constructor(config) {
    ...
    this.genderCondition = config.genderCondition || "M";
    ...
    this.screens = {
      ...
      demographicTransition: document.getElementById("demographic-transition-screen"),
      videoQuality: document.getElementById("video-quality-screen"),
      demographicQuestions: document.getElementById("demographic-questions-screen"),
    ...
    this.demographicQuestions = [
      {
        key: "sex",
        text: "Which of the following best describes your sex?",
        options: [
          { value: "1", label: "Male" },
      ...
      },
      {
        ...
        options: [
          { value: "1", label: "Asian / Asian American" },
          { value: "2", label: "Black / African American" },
          { value: "3", label: "Latino / Latin American" },
          { value: "4", label: "White / European American" },
          { value: "5", label: "Biracial or Other" }
        ]
      },
      {
        key: "age",
        text: "What is your age? (Press Enter/Return or click the Continue button below when you are ready to continue)",
        type: "text"
      },
      {
        key: "political",
        text: "Which of the following best describes your political orientation?",
        options: [
          { value: "1", label: "Very Conservative" },
          { value: "2", label: "Conservative" },
          { value: "3", label: "Moderate" },
          { value: "4", label: "Liberal" },
          { value: "5", label: "Very Liberal" }
        ]
      },
      {
        key: "sexOrient",
        ...
        options: [
          { value: "1", label: "Heterosexual / Straight" },
          { value: "2", label: "Homosexual / Gay / Lesbian" },
      ...
      {
        key: "education",
        text: "Which of the following best describes your educational background?",
        options: [
          { value: "1", label: "Less than high school diploma" },
          { value: "2", label: "High school diploma" },
          { value: "3", label: "Some college" },
          { value: "4", label: "Associates degree" },
          { value: "5", label: "Bachelors degree" },
          { value: "6", label: "Some graduate school" },
          { value: "7", label: "Masters / Doctorate" }
        ]
      }
    ];

    this.currentDemographicIndex = 0;
    this.demographicResponses = {};
  ...
  _wireEvents() {
    ...
    document.getElementById("demographic-transition-btn").addEventListener("click", () => {
      this.showScreen("videoQuality");
      this.phase = "videoQuality";
    });

    document.addEventListener("keydown", (event) => {
      if (this.phase === "videoQuality") {
        this.handleVideoQualityKeypress(event).catch(err => alert(`Experiment error: ${err.message || err}`));
      } else if (this.phase === "demographicQuestions") {
        this.handleDemographicKeypress(event).catch(err => alert(`Experiment error: ${err.message || err}`));
      }
    });
  ...
  promptForProlificId() {
    let enteredId = "";
    while (!enteredId.trim()) {
      enteredId = window.prompt("Please enter your Prolific ID:", "");
      if (enteredId === null) {
        return;
      }
      if (!enteredId.trim()) {
        alert("Prolific ID is required. Please enter a valid ID.");
      }
    }
    this.data.participantId = enteredId.trim();
  ...
  async advanceToNextTrial() {
    ...
    if (this.currentTrialIndex >= this.trials.length) {
      this.showScreen("demographicTransition");
      return;
    }

    const trialDef = this.trials[this.currentTrialIndex];
    const nextBlockId = trialDef.meta.blockId;

    if (this.currentBlockId !== null && nextBlockId !== this.currentBlockId) {
      this.showScreen("break");
      this.currentBlockId = nextBlockId;
      this.currentTrialIndex -= 1; // Don't skip the first trial of the new block
      return;
    }
  ...
  async handleVideoQualityKeypress(event) {
    ...
    this.demographicResponses.videoQuality = String(numericValue);
    this.showDemographicQuestions();
  }

  showDemographicQuestions() {
    this.showScreen("demographicQuestions");
    this.currentDemographicIndex = 0;
    if (!this.demographicResponses) {
      this.demographicResponses = {};
    }
    this.showCurrentDemographicQuestion();
  }

  showCurrentDemographicQuestion() {
    const container = document.getElementById("demographic-questions-container");
    const question = this.demographicQuestions[this.currentDemographicIndex];

    if (!question) {
      this.data.demographics = this.demographicResponses;
      this.finishExperiment().catch(err => alert(`Experiment error: ${err.message || err}`));
      return;
    }
    ...
    if (question.type === "text") {
      html += `<input type="text" id="demographic-input" style="padding: 10px; font-size: 16px; width: 200px;" autofocus>`;
      html += `<div style="margin-top: 15px;"><button id="demographic-continue-btn" style="padding: 10px 20px; font-size: 16px; cursor: pointer;">Continue</button></div>`;
    } else {
      html += `<div style="margin-top: 15px;">`;
      question.options.forEach((opt, idx) => {
        html += `<div style="margin: 5px 0;">${idx + 1} - ${opt.label}</div>`;
      });
      html += `</div>`;
      html += `<p style="margin-top: 15px; color: #666;">Press a number key to respond</p>`;
    }

    container.innerHTML = html;

    if (question.type === "text") {
      const input = document.getElementById("demographic-input");
      const continueBtn = document.getElementById("demographic-continue-btn");
      input.focus();

      const submitAnswer = () => {
        this.demographicResponses[question.key] = input.value;
        this.currentDemographicIndex += 1;
        this.showCurrentDemographicQuestion();
      };
  ...
  async handleDemographicKeypress(event) {
    const question = this.demographicQuestions[this.currentDemographicIndex];
    ...
    const numericValue = Number(key);
    if (numericValue < 1 || numericValue > question.options.length) return;

    const selectedOption = question.options[numericValue - 1];
    this.demographicResponses[question.key] = selectedOption.value;
  ...
  async finishExperiment() {
    ...
    if (this.config.saveToGoogleDrive) {
      try {
        ...
        setTimeout(() => {
          window.location.href = "https://app.prolific.com/submissions/complete?cc=CTCLOG36";
        }, 3000);
      ...
      setTimeout(() => {
        window.location.href = "https://app.prolific.com/submissions/complete?cc=C11GKFJ2";
      }, 3000);
    }
  }
  ...
  makeCSVText() {
    ...
    const headers = [
      ...
      "genderCondition",
      "trialIndex", "withinBlockTrialIndex", "trialId", "blockId", "blockOrder", "conditionIndex", "ratioLabel", "numTyp", "numAtyp",
      "scaleOrder", "videoOrder", "questionIndex", "scaleKey", "questionText", "questionImage",
      "response", "rtMS", "powerAvg", "statusAvg"
    ];
    ...
    for (const t of this.data.trials) {
      ...
      const videoOrder = (t.videoOrder || [])
        .map(v => `${v.slotIndex + 1}:${v.gender}:${v.typicality}:${v.url}`)
        .join("|");

      const powerResponses = t.responses.filter(r => r.scaleKey && r.scaleKey.startsWith("power"));
      const statusResponses = t.responses.filter(r => r.scaleKey && r.scaleKey.startsWith("status"));

      const powerAvg = powerResponses.length > 0
        ? powerResponses.reduce((sum, r) => sum + Number(r.value || 0), 0) / powerResponses.length
        : "";
      const statusAvg = statusResponses.length > 0
        ? statusResponses.reduce((sum, r) => sum + Number(r.value || 0), 0) / statusResponses.length
        : "";
    ...
    if (this.data.demographics) {
      const demoKeys = ["videoQuality", "sex", "race", "age", "political", "sexOrient", "education"];
      const demoLabels = {
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    // Randomly assign participant to male or female condition
    const genderCondition = Math.random() < 0.5 ? "M" : "F";
    console.log(`Participant assigned to gender condition: ${genderCondition}`);

    const built = await buildExperimentTrials(EXPERIMENT_CONFIG, genderCondition);
    const startTrialIndex = getDeveloperStartIndex(EXPERIMENT_CONFIG, built.trials);
    const config = { ...EXPERIMENT_CONFIG, trials: built.trials, buildInfo: built.buildInfo, startTrialIndex, genderCondition };
